...
del_old_file() {
	...
	rm -rf $_v2ray_sh $is_old_conf $is_old_dir $is_core_dir/*_v2ray_config.json /usr/bin/v2ray
	# del alias
...
if [[ $is_old_use ]]; then
	...
	if [[ $caddy ]]; then
		get install-caddy
		# bak caddy files
		mv -f $is_caddyfile $is_caddyfile.bak
		mv -f $is_caddy_dir/sites $is_caddy_dir/sites.bak
		load caddy.sh
		caddy_config new
	fi
	is_change=1
