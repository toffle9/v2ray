...
show_help() {
    ...
    echo -e "  -p, --proxy <addr>              使用代理下载, e.g., -p http://127.0.0.1:1080 or -p socks5://127.0.0.1:1080"
...
pass_args() {
    while [[ $# -gt 0 ]]; do
        ...
        -p | --proxy)
            [[ -z $2 ]] && {
                err "($1) 缺少必需参数, 正确使用示例: [$1 http://127.0.0.1:1080 or -p socks5://127.0.0.1:1080]"
            }
            proxy=$2
            shift 2
            ;;
